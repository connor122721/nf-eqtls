...
foreach(i=1:30) %do% {
  ...
  PClist <- paste("Dim.", seq(1, i), sep="")
  
  # RNA
  rnai <- data.frame(fin[SAMPLE_ID_NWD %in% dna$sample] %>% 
                  select(sample, SAMPLE_ID_NWD, PClist, Gender, Affected_NF))
  
  # Merging - first round
  fin.dt <- data.table(rnai[-1] %>% 
                left_join(meta.pca %>% select(-c(sample)), 
                ...
                left_join(dna, 
                 ...
                 mutate(Gender=case_when(Gender == "Female" ~ 0,
                                         Gender == "Male" ~ 1,
                                         Gender == "" ~ 2),
                        Affected_NF=case_when(Affected_NF=="Affected" ~ 1,
                                              Affected_NF=="Nonfailing" ~ 0)) %>% 
                  select(-c(diagnosis_simple), sample=SAMPLE_ID_NWD))
  
  # Split dataset into control and failing
  fin.aff <- fin.dt[Affected_NF==1] %>% select(-c(Affected_NF))
  fin.non <- fin.dt[Affected_NF==0] %>% select(-c(Affected_NF))

  # Remove samples with no age / sex # Affected group
  fini.aff <- t(na.omit(fin.aff))
  colnames(fini.aff) <- fini.aff[1,]
  fin1.aff <- data.table(fini.aff[-1,])
  fin1.aff$x <- rownames(fini.aff[-1,])
  
  # Non-failing - controls
  fini.non <- t(na.omit(fin.non))
  colnames(fini.non) <- fini.non[1,]
  fin1.non <- data.table(fini.non[-1,])
  fin1.non$x <- rownames(fini.non[-1,])
  
  # Write covariates object
  write.table(x = fin1.aff %>% 
                select(c(x, contains("NWD"))), 
              file = paste("topchef_cov_RNApc1_", i, "_affected_1.15.25.txt", sep=""), 
              sep = "\t", quote = F, row.names = F)

  write.table(x = fin1.non %>% 
                select(c(x, contains("NWD"))), 
              file = paste("topchef_cov_RNApc1_", i, "_nonfailing_1.15.25.txt", sep=""), 
              sep = "\t", quote = F, row.names = F)
